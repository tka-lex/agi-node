...
AGIChannel.prototype.answer = function() {
  const result = this.sendRequest('ANSWER');
...
AGIChannel.prototype.channelStatus = function(channelName) {
  ...
  const result = this.sendRequest(sprintf('CHANNEL STATUS %s', channelName));
...
AGIChannel.prototype.getData = function(file, timeout, maxDigits) {
  ...
  const result = this.sendRequest(sprintf('GET DATA "%s" %s %s', file, timeout,
    maxDigits));
...
AGIChannel.prototype.getFullVariable = function(variable, channel) {
  ...
  const result = this.sendRequest(sprintf('GET FULL VARIABLE %s %s', variable, channel));
...
AGIChannel.prototype.getVariable = function(variable) {
  const result = this.sendRequest(sprintf('GET VARIABLE "%s"', variable));
